...
class InvoiceGenerationService:
    ...
    def _generate_line_items(
        ...
        line_items.append({
            "description": f"Base Subscription - Q{self._get_quarter_from_periods(period_numbers)}",
    ...
    async def generate_quarterly_invoice(
        ...
        This is the main entry point for quarterly invoice generation.
        ...
        # Step 3: Check if usage periods exist for this quarter
        usage_periods = subscription.get("usage_periods", [])
        available_periods = [p.get("period_number") for p in usage_periods if p.get("period_number") in period_numbers]

        if not available_periods:
            logger.warning(f"[INVOICE_GEN] No usage periods found for Q{quarter} (periods {period_numbers})")
            # We can still generate an invoice with just the base charge

        # Step 4: Generate billing period
        ...
        # Validate line items with Pydantic
